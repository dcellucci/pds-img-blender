...
import os
from ..pvl import dump, dumps, encoder, load, PVLModule, Units
import collections
...
class Pointer(collections.namedtuple('Pointer', ['filename', 'bytes'])):
    ...
    def _parse_bytes(value, record_bytes):
        ...
        if isinstance(value, Units) and value.units == 'BYTES':
            return value.value
...
class PDS3Image(PlanetaryImage):
    ...
    def _save(self, file_to_write, overwrite):
        ...
        encoder = encoder.PDSLabelEncoder
        serial_label = dumps(self.label, cls=encoder)
        ...
        diff = 0
        if len(dumps(self.label, cls=encoder)) != label_sz:
            diff = abs(label_sz - len(dumps(self.label, cls=encoder)))
        dump(self.label, file_to_write, cls=encoder)
    ...
    def _update_label(self, label, array):
        ...
        encoder = encoder.PDSLabelEncoder
        serial_label = dumps(label, cls=encoder)
        label_sz = len(serial_label)
